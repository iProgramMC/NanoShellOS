...
void P_InitPicAnims (void)
{
    ...
    for (i=0 ; animdefs[i].istexture != -1 ; i++)
    {
		...
		if (animdefs[i].istexture)
		{
			...
			int picNum  = R_TextureNumForName(endname);
			int basePic = R_TextureNumForName(startname);
			LogMsg("LastAnim: %x", lastanim);//TODO: just doing it without variables breaks it?
			lastanim->picnum  = picNum;
		...
		else
		{
			...
			int picNum  = R_FlatNumForName(endname);
			int basePic = R_FlatNumForName(startname);
			LogMsg("LastAnim: %x", lastanim);//TODO: just doing it without variables breaks it?
			lastanim->picnum  = picNum;
			lastanim->basepic = basePic;
		}
...
static void DonutOverrun(fixed_t *s3_floorheight, short *s3_floorpic,
                         __attribute__((unused)) line_t *line, __attribute__((unused)) sector_t *pillar_sector)
{
    static int first = 1;
