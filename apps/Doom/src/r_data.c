...
void R_InitTextures (void)
{
	...
	if (!patchlookup)
	{
		I_Error("PatchLookup couldn't allocate!!! NumMapPatches: %d sizeof(*patchLookup): %d", nummappatches, sizeof(*patchlookup));
	}

	//LogMsg("NumMapPatches: %d sizeof(*patchLookup): %d", nummappatches, sizeof(*patchlookup));
    for (i = 0; i < nummappatches; i++)
    {
        M_StringCopy(name, name_p + i * 8, sizeof(name));
        int num = W_CheckNumForName(name);
		//LogMsg("SETTING PATCHLOOKUP=%x I=%d",patchlookup,i);
		patchlookup[i] = num;
    }
