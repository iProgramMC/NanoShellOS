...
static int GetFileNumber(wad_file_t *handle)
{
	...
	wad_file_t** oldOpenWadFiles = open_wadfiles;
	open_wadfiles = malloc(sizeof(wad_file_t *) * (num_open_wadfiles + 1));
	memset(open_wadfiles, 0, sizeof(wad_file_t *) * (num_open_wadfiles + 1));
	
	memcpy (open_wadfiles, oldOpenWadFiles, sizeof(wad_file_t *) * (num_open_wadfiles));
	free (oldOpenWadFiles);
