...
static void check_page_cross_y(nes_cpu_t *nes_cpu)
{
...
static void check_new_page(nes_cpu_t *nes_cpu)
{
...
static void cpu_checknz(nes_cpu_t *nes_cpu, uint8_t n)
{
...
static void cpu_modify_flag(nes_cpu_t *nes_cpu, uint8_t flag, int value) {
    if(value) { nes_cpu->regs.P |= flag; }
    else      { nes_cpu->regs.P &= ~flag; }
}

static void cpu_stack_push_byte(nes_cpu_t *nes_cpu, uint8_t data) 
{ 
    memory_write_byte(nes_cpu, 0x100 + nes_cpu->regs.S, data); 
    nes_cpu->regs.S -= 1; 
}

static void cpu_stack_push_word(nes_cpu_t *nes_cpu, uint16_t data)    
{ 
    memory_write_word(nes_cpu, 0x0ff + nes_cpu->regs.S, data); 
    nes_cpu->regs.S -= 2; 
}

static uint8_t cpu_stack_pop_byte(nes_cpu_t *nes_cpu)              
{ 
    nes_cpu->regs.S += 1; 
    return memory_read_byte(nes_cpu, 0x100 + nes_cpu->regs.S); 
}

static uint16_t cpu_stack_pop_word(nes_cpu_t *nes_cpu)              
{
    nes_cpu->regs.S += 2; 
    return memory_read_word(nes_cpu, 0x0ff + nes_cpu->regs.S); 
}

static void cpu_addressing_implied(UNUSED nes_cpu_t *nes_cpu) 
{ 
}

static void cpu_addressing_accumulator(UNUSED nes_cpu_t *nes_cpu) 
{ 
}

static void cpu_addressing_immediate(nes_cpu_t *nes_cpu) 
{
    nes_cpu->op_value = memory_read_byte(nes_cpu, nes_cpu->regs.PC);
    nes_cpu->regs.PC++;
}

static void cpu_addressing_zeropage(nes_cpu_t *nes_cpu) 
{
...
static void cpu_addressing_zeropage_x(nes_cpu_t *nes_cpu) 
{
...
static void cpu_addressing_zeropage_y(nes_cpu_t *nes_cpu) 
{
...
static void cpu_addressing_absolute(nes_cpu_t *nes_cpu) 
{
...
static void cpu_addressing_absolute_x(nes_cpu_t *nes_cpu) 
{
...
static void cpu_addressing_absolute_y(nes_cpu_t *nes_cpu) 
{
...
static void cpu_addressing_relative(nes_cpu_t *nes_cpu) 
{
    nes_cpu->op_address = memory_read_byte(nes_cpu, nes_cpu->regs.PC);
    nes_cpu->regs.PC++;
...
static void cpu_addressing_indirect(nes_cpu_t *nes_cpu) 
{
...
static void cpu_addressing_indirect_x(nes_cpu_t *nes_cpu) 
{
...
static void cpu_addressing_indirect_y(nes_cpu_t *nes_cpu) 
{
    uint8_t arg_addr = memory_read_byte(nes_cpu, nes_cpu->regs.PC);
...
/* ALU instructions */
static void cpu_ora(nes_cpu_t *nes_cpu) 
{
    nes_cpu->regs.A |= nes_cpu->op_value;
    cpu_checknz(nes_cpu, nes_cpu->regs.A);
}

static void cpu_and(nes_cpu_t *nes_cpu)
{
    nes_cpu->regs.A &= nes_cpu->op_value;
    cpu_checknz(nes_cpu, nes_cpu->regs.A);
}

static void cpu_eor(nes_cpu_t *nes_cpu) 
{
    nes_cpu->regs.A ^= nes_cpu->op_value;
    cpu_checknz(nes_cpu, nes_cpu->regs.A);
}

static void cpu_asl(nes_cpu_t *nes_cpu) 
{
...
static void cpu_asla(nes_cpu_t *nes_cpu) 
{
...
static void cpu_rol(nes_cpu_t *nes_cpu) 
{
...
static void cpu_rola(nes_cpu_t *nes_cpu) 
{
    uint8_t tmp = nes_cpu->regs.P & FLAG_CARRY;
    cpu_modify_flag(nes_cpu, FLAG_CARRY, nes_cpu->regs.A & 0x80);
    nes_cpu->regs.A <<= 1;
...
static void cpu_ror(nes_cpu_t *nes_cpu) 
{
    uint8_t tmp = nes_cpu->regs.P & FLAG_CARRY;
...
static void cpu_rora(nes_cpu_t *nes_cpu) 
{
    uint8_t tmp = nes_cpu->regs.P & FLAG_CARRY;
...
static void cpu_lsr(nes_cpu_t *nes_cpu) 
{
    cpu_modify_flag(nes_cpu, FLAG_CARRY, nes_cpu->op_value & 0x01);
    nes_cpu->op_value >>= 1;
    memory_write_byte(nes_cpu, nes_cpu->op_address, nes_cpu->op_value);
...
static void cpu_lsra(nes_cpu_t *nes_cpu) 
{
    cpu_modify_flag(nes_cpu, FLAG_CARRY, nes_cpu->regs.A & 0x01);
    nes_cpu->regs.A >>= 1;
    cpu_checknz(nes_cpu, nes_cpu->regs.A);
...
static void cpu_adc(nes_cpu_t *nes_cpu) 
{
    uint16_t tmp;
...
static void cpu_sbc(nes_cpu_t *nes_cpu) 
{
    uint16_t tmp;
...
static void cpu_bcs(nes_cpu_t *nes_cpu) 
{ 
...
static void cpu_beq(nes_cpu_t *nes_cpu) 
{
...
static void cpu_bvs(nes_cpu_t *nes_cpu) 
{
...
static void cpu_bpl(nes_cpu_t *nes_cpu)
{
    if(!(nes_cpu->regs.P & FLAG_NEGATIVE)) 
    {
        check_new_page(nes_cpu);
...
static void cpu_bcc(nes_cpu_t *nes_cpu)
{
    if(!(nes_cpu->regs.P & FLAG_CARRY))
    {
        check_new_page(nes_cpu);
...
static void cpu_bne(nes_cpu_t *nes_cpu)
{
    if(!(nes_cpu->regs.P & FLAG_ZERO)) 
    {
        check_new_page(nes_cpu);
...
static void cpu_bvc(nes_cpu_t *nes_cpu)
{
    if(!(nes_cpu->regs.P & FLAG_OVERFLOW)) 
    {
        check_new_page(nes_cpu);
...
/* Compare */
static void cpu_bit(nes_cpu_t *nes_cpu) 
{
...
static void cpu_cmp(nes_cpu_t *nes_cpu) 
{
...
static void cpu_cpx(nes_cpu_t *nes_cpu) 
{
...
static void cpu_cpy(nes_cpu_t *nes_cpu) 
{
...
static void cpu_sei(nes_cpu_t *nes_cpu) { cpu_modify_flag(nes_cpu, FLAG_INTERRUPT, 1); }
static void cpu_sed(nes_cpu_t *nes_cpu) { cpu_modify_flag(nes_cpu, FLAG_DECIMAL, 1); }

static void cpu_dec(nes_cpu_t *nes_cpu) {
    nes_cpu->op_value -= 1;
    memory_write_byte(nes_cpu, nes_cpu->op_address, nes_cpu->op_value);
    cpu_checknz(nes_cpu, nes_cpu->op_value);
}

static void cpu_dex(nes_cpu_t *nes_cpu) {
    nes_cpu->regs.X--;
    cpu_checknz(nes_cpu, nes_cpu->regs.X);
}

static void cpu_dey(nes_cpu_t *nes_cpu) {
    nes_cpu->regs.Y--;
    cpu_checknz(nes_cpu, nes_cpu->regs.Y);
}

static void cpu_inc(nes_cpu_t *nes_cpu) {
    nes_cpu->op_value += 1;
    memory_write_byte(nes_cpu, nes_cpu->op_address, nes_cpu->op_value);
    cpu_checknz(nes_cpu, nes_cpu->op_value);
}

static void cpu_inx(nes_cpu_t *nes_cpu) {
    nes_cpu->regs.X++;
    cpu_checknz(nes_cpu, nes_cpu->regs.X);
}

static void cpu_iny(nes_cpu_t *nes_cpu) {
    nes_cpu->regs.Y++;
    cpu_checknz(nes_cpu, nes_cpu->regs.Y);
}

static void cpu_lda(nes_cpu_t *nes_cpu) 
{
    nes_cpu->regs.A = nes_cpu->op_value; 
    cpu_checknz(nes_cpu, nes_cpu->regs.A); 
}

static void cpu_ldx(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.X = nes_cpu->op_value; 
    cpu_checknz(nes_cpu, nes_cpu->regs.X); 
}
static void cpu_ldy(nes_cpu_t *nes_cpu) 
{
    nes_cpu->regs.Y = nes_cpu->op_value; 
    cpu_checknz(nes_cpu, nes_cpu->regs.Y); 
}

static void cpu_axs(nes_cpu_t *nes_cpu) 
{ 
    memory_write_byte(nes_cpu, nes_cpu->op_address, nes_cpu->regs.A & nes_cpu->regs.X); 
}

static void cpu_sta(nes_cpu_t *nes_cpu) 
{ 
    memory_write_byte(nes_cpu, nes_cpu->op_address, nes_cpu->regs.A); 
}

static void cpu_stx(nes_cpu_t *nes_cpu) 
{ 
    memory_write_byte(nes_cpu, nes_cpu->op_address, nes_cpu->regs.X); 
}

static void cpu_sty(nes_cpu_t *nes_cpu) 
{ 
    memory_write_byte(nes_cpu, nes_cpu->op_address, nes_cpu->regs.Y); 
}

static void cpu_nop(UNUSED nes_cpu_t *nes_cpu) {}

static void cpu_pha(nes_cpu_t *nes_cpu) 
{ 
    cpu_stack_push_byte(nes_cpu, nes_cpu->regs.A); 
}

static void cpu_php(nes_cpu_t *nes_cpu) 
{ 
    cpu_stack_push_byte(nes_cpu, nes_cpu->regs.P | 0x30); 
}

static void cpu_pla(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.A = cpu_stack_pop_byte(nes_cpu); 
    cpu_checknz(nes_cpu, nes_cpu->regs.A); 
}

static void cpu_plp(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.P = (cpu_stack_pop_byte(nes_cpu) & 0xef) | 0x20; 
}

static void cpu_rts(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.PC = cpu_stack_pop_word(nes_cpu) + 1; 
}
static void cpu_rti(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.P = cpu_stack_pop_byte(nes_cpu) | FLAG_UNUSED; 
    nes_cpu->regs.PC = cpu_stack_pop_word(nes_cpu); 
}

static void cpu_jmp(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.PC = nes_cpu->op_address; 
}

static void cpu_jsr(nes_cpu_t *nes_cpu) 
{ 
    cpu_stack_push_word(nes_cpu, nes_cpu->regs.PC - 1); 
    nes_cpu->regs.PC = nes_cpu->op_address; 
}

static void cpu_brk(nes_cpu_t *nes_cpu) 
{
    cpu_stack_push_word(nes_cpu, nes_cpu->regs.PC - 1);
...
static void cpu_tax(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.X = nes_cpu->regs.A; 
    cpu_checknz(nes_cpu, nes_cpu->regs.X); 
}

static void cpu_tay(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.Y = nes_cpu->regs.A; 
    cpu_checknz(nes_cpu, nes_cpu->regs.Y); 
}

static void cpu_txa(nes_cpu_t *nes_cpu) 
{
    nes_cpu->regs.A = nes_cpu->regs.X; 
    cpu_checknz(nes_cpu, nes_cpu->regs.A); 
}

static void cpu_tya(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.A = nes_cpu->regs.Y; 
    cpu_checknz(nes_cpu, nes_cpu->regs.A); 
}

static void cpu_tsx(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.X = nes_cpu->regs.S; cpu_checknz(nes_cpu, nes_cpu->regs.X); 
}

static void cpu_txs(nes_cpu_t *nes_cpu) 
{ 
    nes_cpu->regs.S = nes_cpu->regs.X; 
}

uint64_t cpu_clock(nes_cpu_t *nes_cpu) 
{
