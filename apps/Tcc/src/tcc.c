...
const char * g_CIncludePath = "/User/Include";
const char * g_CLibraryPath = "/User/Library";

char* TCC_getenv(const char* name)
{
	// TODO: Call into the system and grab info from ns.ini. For now, hardcoded.
	if (strcmp(name, "C_INCLUDE_PATH") == 0)
	{
		return (char*) g_CIncludePath;
	}
	if (strcmp(name, "LIBRARY_PATH") == 0)
	{
		return (char*) g_CLibraryPath;
	}
...
static void set_environment(TCCState *s)
{
    ...
    path = TCC_getenv("C_INCLUDE_PATH");
    if(path != NULL) {
        tcc_add_sysinclude_path(s, path);
    }
    path = TCC_getenv("CPATH");
    if(path != NULL) {
        tcc_add_include_path(s, path);
    }
    path = TCC_getenv("LIBRARY_PATH");
    if(path != NULL) {
