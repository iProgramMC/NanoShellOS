...
# endif
        ...
        tcc_add_library_err(s1, "nanoshell");
#ifdef TCC_LIBGCC
        if (!s1->static_link) {
            if (TCC_LIBGCC[0] == '/')
                           ...
                           int file_offset, int *sec_order)
{
    ...
    if (file_type == TCC_OUTPUT_OBJ) {
        ...
        if (s1->elf_entryname)
            ...
            ehdr.e_entry = get_sym_addr(s1, "_NsStart", !!(file_type & TCC_OUTPUT_EXE), 0);
	...
	resolve_common_syms(s1);
        ...
        if (!s1->static_link) {
            if (file_type & TCC_OUTPUT_EXE) {
                ...
                const char *elfint = TCC_getenv("LD_SO");
                if (elfint == NULL)
