...
__attribute__((noreturn)) void KiStartupSystem(unsigned long check,
  ...
  if (check != 0x2badb002) {
    /*SwitchMode(0);
    CoInitAsText(&g_debugConsole);*/
    LogMsg("NanoShell has not booted from a Multiboot-compatible bootloader.  "
           "A bootloader such as GRUB is required to run NanoShell.");
    KeStopSystem();
  }
