...
uint32_t MrGetReferenceCount(uintptr_t page)
{
	...
	// Is there a level1?
	if (!g_root2.m_level1[aSplit.level1])
	{
	...
	// There is a level1.
	return g_root2.m_level1[aSplit.level1]->m_refCounts[aSplit.level2];
}
...
uint32_t MrReferencePage(uintptr_t page)
{
	...
	if (!g_root2.m_level1[aSplit.level1])
	{
		...
		// 4m cluster
		g_root2.m_level1[aSplit.level1] = MhAllocateSinglePage(NULL);
		
		RSLogMsg("g_root2.m_level1[%d] = %p", aSplit.level1, g_root2.m_level1[aSplit.level1]);
		...
		memset(g_root2.m_level1[aSplit.level1], 0, PAGE_SIZE);
	}
	
	uint32_t* pRefCount = &g_root2.m_level1[aSplit.level1]->m_refCounts[aSplit.level2];
...
uint32_t MrUnreferencePage(uintptr_t page)
{
	...
	if (!*pRefCount)
	{
		...
		{
			if (g_root2.m_level1[aSplit.level1]->m_refCounts[i] != 0)
			{
				bNotAllZeroes = true;
				break;
			}
		...
		if (!bNotAllZeroes)
		{
			...
			MhFree(g_root2.m_level1[aSplit.level1]);
			g_root2.m_level1[aSplit.level1] = NULL;
		}
	}
...
void MrDebug()
{
	for (int i = 0; i < 1024; i++)
	{
		if (g_root2.m_level1[i])
		{
			for (int j = 0; j < 1024; j++)
			{
				if (g_root2.m_level1[i]->m_refCounts[j])
					LogMsg("Page %x has ref count %d", i<<22|j<<12, g_root2.m_level1[i]->m_refCounts[j]);
			}
		}
	}
}
